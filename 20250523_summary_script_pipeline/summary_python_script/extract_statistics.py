...
def get_insert_size(bam_file):
    ...
    try:
        ...
        cmd = (
            ...
            "grep '^SN' | grep 'insert size average:' | cut -f 3"
        )
...
# Get all the statistics for every sample from the list
...
# Look for the files (it's because of the sample number "S##" - we have to handle that as a wildcard)
            ...
            # Ensure total_reads propagates NA correctly
            if stats['mapped_reads'] == "NA" or stats['duplicated_reads'] == "NA":
                stats['duplication_rate'] = "NA"
            else:
                stats['duplication_rate'] = stats['duplicated_reads']/stats['mapped_reads'] * 100


# Print it all to the tsv file:
            print("\t".join(map(str, [
